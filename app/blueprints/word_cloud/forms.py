...
class WordCloudForm(FlaskForm):
    ...
    max_word_count = StringField("Max Word Count", validators=[DataRequired()])

    submit = SubmitField("Track Topics")

    def validate(self):
        if not super(WordCloudForm, self).validate():
            return False
        ...
        try:
            max_word_count = int(self.max_word_count.data)
        except Exception as e:
            self.max_word_count.errors.append("Please enter an integer.")
            return False
        if max_word_count < 50:
            self.max_word_count.errors.append("Please enter an integer greater than 50.")
            return False
